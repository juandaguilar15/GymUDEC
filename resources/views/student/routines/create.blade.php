...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .container { max-width: 800px; margin: 2rem auto; padding: 2rem; background: white; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
        h1 { color: #1B5E46; font-size: 24px; margin-bottom: 25px; }
        .form-group { margin-bottom: 20px; }
        label { display: block; font-weight: 600; color: #1B5E46; margin-bottom: 8px; font-size: 14px; }
        input[type="text"], input[type="number"], select, textarea { width: 100%; padding: 12px; border: 2px solid #e0e0e0; border-radius: 5px; font-family: 'Poppins', sans-serif; font-size: 14px; transition: border-color 0.3s; }
        input[type="text"]:focus, input[type="number"]:focus, select:focus, textarea:focus { outline: none; border-color: #1B5E46; box-shadow: 0 0 0 3px rgba(27, 94, 70, 0.1); }
        textarea { resize: vertical; min-height: 100px; }
        .form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
        .errors { margin-bottom: 20px; }
        .errors ul { list-style: none; padding: 0; }
        .errors li { background: #f8d7da; color: #721c24; padding: 10px; border-radius: 5px; margin-bottom: 8px; border-left: 4px solid #f5c6cb; font-size: 14px; }
        .days-selector { display: grid; grid-template-columns: repeat(auto-fit, minmax(100px, 1fr)); gap: 10px; margin-bottom: 10px; }
        .day-button { padding: 10px; border: 2px solid #e0e0e0; background: white; border-radius: 5px; cursor: pointer; font-weight: 600; font-size: 13px; transition: all 0.3s; }
        .day-button:hover { border-color: #1B5E46; background: #f0f8f5; }
        .day-button.selected { background: #1B5E46; color: white; border-color: #1B5E46; }
        .info-text { font-size: 12px; color: #666; margin-top: 5px; }
        .exercises-by-day { background: #f9f9f9; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #F8B803; }
        .section-title { color: #1B5E46; font-weight: 600; margin-bottom: 10px; display: flex; align-items: center; gap: 8px; }
        .day-tabs { display: flex; gap: 8px; margin-bottom: 15px; flex-wrap: wrap; border-bottom: 2px solid #e0e0e0; padding-bottom: 10px; }
        .day-tab { padding: 8px 15px; background: white; border: none; border-radius: 3px 3px 0 0; cursor: pointer; font-weight: 600; font-size: 13px; color: #666; transition: all 0.3s; }
        .day-tab:hover { background: #f0f0f0; }
        .day-tab.active { background: #1B5E46; color: white; }
        .day-content { display: none; }
        .day-content.active { display: block; }
        .exercise-item { background: white; padding: 15px; margin-bottom: 15px; border-radius: 5px; border: 1px solid #e0e0e0; }
        .exercise-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; }
        .remove-exercise { background: #e74c3c; color: white; border: none; padding: 5px 10px; border-radius: 3px; cursor: pointer; font-size: 12px; font-weight: 600; transition: background 0.3s; }
        .remove-exercise:hover { background: #c0392b; }
        .exercise-fields { display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 10px; }
        .button-group { display: flex; gap: 10px; margin-top: 30px; }
        .submit-btn { flex: 1; padding: 12px; background: #F8B803; color: white; border: none; border-radius: 5px; font-weight: 600; cursor: pointer; font-size: 16px; transition: background 0.3s; }
        .submit-btn:hover { background: #e6a700; }
        .back-btn { flex: 1; padding: 12px; background: #e0e0e0; color: #333; border: none; border-radius: 5px; font-weight: 600; cursor: pointer; font-size: 16px; transition: background 0.3s; text-decoration: none; display: flex; align-items: center; justify-content: center; }
        .back-btn:hover { background: #d0d0d0; }
        .add-exercise-btn { background: #1B5E46; color: white; padding: 10px 15px; border: none; border-radius: 5px; cursor: pointer; font-weight: 600; margin-top: 10px; transition: background 0.3s; font-size: 13px; }
        .add-exercise-btn:hover { background: #2a7a5e; }
        .required { color: #e74c3c; }
        @media (max-width: 768px) { .form-row { grid-template-columns: 1fr; } .button-group { flex-direction: column; } .exercise-fields { grid-template-columns: 1fr; } .days-selector { grid-template-columns: repeat(auto-fit, minmax(80px, 1fr)); } }
    </style>
</head>
<body>
    <div class="container">
        <h1>📋 Crear Mi Rutina</h1>
        ...
        <form action="{{ route('student.routines.store') }}" method="POST" id="routineForm">
            @csrf

            <div class="form-row">
                <div class="form-group">
                    <label for="name">Nombre de la Rutina <span class="required">*</span></label>
                    <input type="text" id="name" name="name" placeholder="Ej: Rutina de Fuerza Principiante" value="{{ old('name') }}" required>
                </div>
                <div class="form-group">
                    <label for="level">Nivel <span class="required">*</span></label>
                    <input type="text" id="level" name="level" placeholder="Ej: Principiante, Intermedio, Avanzado" value="{{ old('level') }}" required>
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="duration_weeks" name="duration_weeks" min="1" max="52" value="{{ old('duration_weeks') }}" required>
                </div>
                <div class="form-group">
                    <label for="days_per_week">Días por Semana <span class="required">*</span></label>
                    <input type="number" id="days_per_week" name="days_per_week" min="1" max="7" value="{{ old('days_per_week') }}" required onchange="updateDaySelector()">
            ...
            <div class="form-group">
                ...
                <textarea id="description" name="description" placeholder="Describe la rutina, objetivos específicos, recomendaciones..." required>{{ old('description') }}</textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="days-selector" id="daysSelector">
                    @foreach ($days_of_week as $dayValue => $dayLabel)
                        <button type="button" class="day-button" data-day="{{ $dayValue }}" data-day-label="{{ $dayLabel }}" onclick="toggleDay(this, event)">
                            {{ $dayLabel }}
                        </button>
                    @endforeach
                </div>
            ...
            <div class="button-group">
                ...
                <a href="{{ route('student.routines.index') }}" class="back-btn">← Volver</a>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        function updateDaySelector() {
            const daysPerWeek = parseInt(document.getElementById('days_per_week').value) || 1;
            document.getElementById('selectedDaysInfo').textContent = `Debes seleccionar ${daysPerWeek} día(s). Seleccionados: ${selectedDays.length}/${daysPerWeek}`;
        }
                    ...
                    <div class="form-group">
                        ...
                        <select name="exercises[]" class="exercise-select" required onchange="onExerciseSelectionChange(this, '${uniqueId}')">
                            ...
                            @foreach ($exercises as $exercise)
                                <option value="{{ $exercise->id }}" data-format="{{ $exercise->exercise_format ?? 'series_reps' }}">{{ $exercise->name }} ({{ $exercise->type }})</option>
                            @endforeach
                        </select>
                    </div>
        ...
        function removeExercise(uniqueId, event) {
            event.preventDefault();
            const element = document.getElementById(`exercise-${uniqueId}`);
            if (element) {
                element.remove();
            }
            updateRemoveButtons();
